...
namespace AForge.Video.DirectShow
{
    ...
    public class VideoCaptureDevice : IVideoSource
    {
        ...
        /// <returns></returns>
        private List<Size> GetCaptureSupportSize(ICaptureGraphBuilder2 capGraph, IBaseFilter captureFilter)
        {
            object streamConfig;
            // 获取配置接口
            ...
            // 判断为正确获取的信息
            List<Size> SupportSizeList = new List<Size>();
            if (Marshal.SizeOf(vscc) == iSize)
            {
                for (int i = 0; i < iCount; ++i)
                {
                    ...
                    hr = videoStreamConfig.GetStreamCaps(i, out amMediaType, vscc);
                    
                    // 如果为视频信息
                    if (amMediaType.MajorType == MediaType.Video )
                        //&&
                        ////amMediaType.formatType == FormatType.VideoInfo
                        //amMediaType.FormatType == MediaType.Video )
                    {
                        Marshal.StructureToPtr(vscc, pVscc, false);

                        var videoInfoHeader = new VideoInfoHeader();
                        object ob = Marshal.PtrToStructure(amMediaType.FormatPtr, typeof(VideoInfoHeader));
                        videoInfoHeader = (VideoInfoHeader)ob;
                        // 获取摄像头所支持的分辨率
                        //int width = videoInfoHeader.BmiHeader.Width;
                        //int height = videoInfoHeader.BmiHeader.Height;
                        Size size = new Size(videoInfoHeader.BmiHeader.Width, videoInfoHeader.BmiHeader.Height);
                        if (!SupportSizeList.Contains(size))
                            SupportSizeList.Add(size);
                    }
                    // 释放非托管内存
        ...
        /// 获取摄像头所支持的分辨率
        /// </summary>
        public List<Size> GetCaptureSupportSize()
        {
            object obj = FilterInfo.CreateFilter(deviceMoniker);
            if (obj == null)
                throw new ApplicationException("Failed creating device object for moniker");

            IBaseFilter captureFilter = (IBaseFilter)obj;
            ...
            return GetCaptureSupportSize(captureGraph, captureFilter);
        }

        #region 是否支持 Video ** Pin设置功能
        /// <summary>
        /// 是否存在Video Capture Pin设置功能
        /// </summary>
        private bool ExistVideoCapturePin(ICaptureGraphBuilder2 capGraph, IBaseFilter captureFilter) 
        {
            object streamConfig;
            int hr = capGraph.FindInterface(PinCategory.Capture,
            ...
            if (hr == 0)
            {
                ISpecifyPropertyPages pPropPages = (ISpecifyPropertyPages)(FilterInfo.CreateFilter(deviceMoniker));

                // get property pages from the property bag
                CAUUID caGUID;
                hr = pPropPages.GetPages(out caGUID);
        ...
        /// <summary>
        /// 是否存在Video Preview Pin设置功能
        /// </summary>
        private bool ExistVideoPreviewPin(ICaptureGraphBuilder2 capGraph, IBaseFilter captureFilter) 
        {
            object streamConfig;
            int hr = capGraph.FindInterface(PinCategory.Preview,
                                            MediaType.Interleaved,
                                            captureFilter,
                                            typeof(IAMStreamConfig).GUID,
                                            out streamConfig);
            if (hr != 0)
                hr = capGraph.FindInterface(PinCategory.Preview,
                                            MediaType.Video,
                                            captureFilter,
                                            typeof(IAMStreamConfig).GUID,
                                            out streamConfig);
        ...
        /// 是否存在Pin设置功能
        /// </summary>
        /// <param name="pinName">Pin的名称
        /// Video Capture Pin
        /// Video Preview Pin
        /// </param>
        /// <returns></returns>
        public bool ExistPin(string pinName) 
        {
            bool result = false;
            ...
            if (pinName == "Video Capture Pin")
                result = ExistVideoCapturePin(captureGraph, captureFilter);
            else if (pinName == "Video Preview Pin")
                result = ExistVideoPreviewPin(captureGraph, captureFilter);
            return result;
        }
        #endregion 
